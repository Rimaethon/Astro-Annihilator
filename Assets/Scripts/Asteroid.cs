...
public class Asteroid : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        if (coll.gameObject.CompareTag("Bullet"))
        {
            ...
            Vector3 asteroidMeasure = transform.localScale;
            asteroidMeasure.x = asteroidMeasure.x / 2;
            asteroidMeasure.y = asteroidMeasure.y / 2;
            transform.localScale = asteroidMeasure;
            CircleCollider2D circleCollider2D = GetComponent<CircleCollider2D>();
            circleCollider2D.radius = circleCollider2D.radius / 2;
            Instantiate(gameObject, transform.position, Quaternion.identity);
            gameObject.GetComponent<Asteroid>();
            gameObject.Asteroid();
            Instantiate(gameObject, transform.position, Quaternion.identity);
